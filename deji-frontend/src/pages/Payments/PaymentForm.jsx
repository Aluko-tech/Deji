...
export default function PaymentForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post("/payments", { invoiceId, amount });
      navigate("/payments");
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 shadow rounded">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <input type="text" placeholder="Invoice ID" className="w-full p-2 border rounded" value={invoiceId} onChange={e => setInvoiceId(e.target.value)} />
