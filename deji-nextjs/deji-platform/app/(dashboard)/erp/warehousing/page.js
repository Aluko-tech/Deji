...
export default function WarehousingPage() {
  ...
  const syncStock = async () => {
    try {
      showMsg("🔄 Syncing products to warehouse...");
      const res = await api.post("/warehouses/sync-stock");
      const d = res.data;
      showMsg(`✅ Synced ${d.synced} products to default warehouse`);
      fetchAll();
    } catch(e) {
      showMsg("❌ Sync failed: " + (e.message || "unknown error"));
    }
  };
  const openEdit = (wh) => {
    setForm({
  ...
  const deleteWarehouse = async (wh) => {
    if (!confirm(`Permanently delete "${wh.name}"? This cannot be undone and will remove all stock records for this location.`)) return;
    try {
  ...
  const submitTransfer = async () => {
    ...
    try {
      await api.post("/warehouses/transfers", {
        ...
        carrier: transfer.carrier, trackingNumber: transfer.trackingNumber, notes: transfer.notes,
      });
  ...
  // Filtered inventory for stock map
        ...
        loading ? (
          ...
          <div className="deji-card p-14 text-center">
            ...
            <p className="text-sm mb-6 max-w-xs mx-auto" style={{ color:"var(--text-muted)" }}>
              Add your first location — local or international — to start tracking stock by location.
            </p>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="relative">
              ...
              <input value={txSearch} onChange={e=>setTxSearch(e.target.value)}
                ...
                className="deji-input pl-8 text-sm" style={{ minWidth:220 }}/>
            </div>
                    ...
                    {filteredTransfers.map((t,idx) => (
                      <tr key={t.id||idx}>
                        ...
                        <td>
                          ...
                          {t.product?.sku && <p className="text-[10px]" style={{ color:"var(--text-muted)" }}>{t.product.sku}</p>}
                        </td>
                        <td>
                          <div className="flex items-center gap-1">
                            <span className="text-xs" style={{ color:"var(--text-muted)" }}>{t.fromWarehouse?.name||"—"}</span>
                            {t.fromWarehouse?.country !== "Nigeria" && <span className="text-[10px]">🌍</span>}
                          </div>
                        </td>
                        <td>
                          <div className="flex items-center gap-1">
                            <span className="text-xs" style={{ color:"var(--text-muted)" }}>{t.toWarehouse?.name||"—"}</span>
                            {t.toWarehouse?.country !== "Nigeria" && <span className="text-[10px]">🌍</span>}
                          </div>
                        </td>
                        ...
                        <td>
                          {Number(t.deliveryFee||0) > 0
                            ? <span className="text-sm font-bold text-orange-400">₦{Number(t.deliveryFee).toLocaleString()}</span>
                            : <span style={{ color:"var(--text-muted)" }}>—</span>}
                        </td>
                        <td>
                          <span className={`badge text-xs ${t.status==="completed"?"badge-green":t.status==="in-transit"?"badge-orange":"badge-blue"}`}>
                            {t.status||"completed"}
                          </span>
                        </td>
                      </tr>
      ...
      {activeTab === "stock-map" && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative">
              ...
              <input value={stockSearch} onChange={e=>setStockSearch(e.target.value)}
                ...
                className="deji-input pl-8 text-sm" style={{ minWidth:200 }}/>
            </div>
            <div className="flex gap-2 flex-wrap">
              {categories.slice(0,8).map(cat => (
                <button key={cat} onClick={()=>setStockCat(cat)}
                  ...
                  style={{
                    background:  stockCat===cat ? "var(--primary)" : "transparent",
                    color:       stockCat===cat ? "#fff" : "var(--text-muted)",
                    borderColor: stockCat===cat ? "var(--primary)" : "var(--border)",
                  }}>
          ...
          {warehouses.length === 0 ? (
            <div className="deji-card p-12 text-center">
              <div className="text-5xl mb-3">🗺</div>
              <p style={{ color:"var(--text-muted)" }}>Add warehouses first to see your stock map.</p>
            </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs" style={{ color:"var(--text-muted)" }}>
                            ...
                            {lowCount > 0 && <span className="text-orange-400 ml-2">· {lowCount} low stock</span>}
                            {outCount > 0 && <span className="text-red-400 ml-2">· {outCount} out of stock</span>}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="hidden sm:flex gap-4 text-center">
                          ...
                          {lowCount > 0 && <div><p className="text-sm font-bold text-orange-400">{lowCount}</p><p className="text-[10px]" style={{color:"var(--text-muted)"}}>Low</p></div>}
                        </div>
                        {isExpanded ? <ChevronUp size={16} style={{color:"var(--text-muted)"}}/> : <ChevronDown size={16} style={{color:"var(--text-muted)"}}/> }
                      </div>
                    </button>

                    {/* Expanded product list grouped by category */}
                    {isExpanded && (
                      <div style={{ borderTop:"1px solid var(--border)" }}>
                        {whStocks.length === 0 ? (
                          <div className="p-8 text-center">
                            <div className="text-3xl mb-2">📦</div>
                            <p className="text-sm" style={{ color:"var(--text-muted)" }}>No stock assigned to this location yet.</p>
                            <button onClick={() => { setTransfer(p=>({...p, toWarehouse: wh.id})); setShowTransfer(true); setError(""); }}
                              className="btn-secondary text-xs mt-3 flex items-center gap-1 mx-auto">
                                      ...
                                      <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                          ...
                                          <div>
                                            ...
                                            <div className="flex items-center gap-2 flex-wrap">
                                              ...
                                              {cf.hasVariants && variants.length > 0 && (
                                        ...
                                        <div className="flex items-center gap-3">
                                          ...
                                          <button onClick={() => { setTransfer(p=>({...p, fromWarehouse: wh.id, productId: prod?.id||""})); setShowTransfer(true); setError(""); }}
                                            className="p-1.5 rounded-lg transition-all" style={{color:"var(--text-muted)"}} title="Transfer this product">
                                        ...
                                        <div className="mt-2 ml-12 flex flex-wrap gap-2">
                                          {variants.map((v,i) => (
                                            <div key={i} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[10px]"
                                              ...
                                              {v.imageUrl && <img src={v.imageUrl} className="w-4 h-4 rounded object-cover"/>}
                                              <span className="font-semibold" style={{color:"var(--text-primary)"}}>{v.name}</span>
                                              <span>· {v.stock||0} units</span>
                                            </div>
                                          ))}
                                        </div>
                                      )}
                            ...
                            <th key={wh.id} className="text-center">
                              <div>
                                <p className="flex items-center justify-center gap-1">
                                  {wh.isDefault && <span className="text-[10px]">⚡</span>}
                                  {wh.country !== "Nigeria" && <span className="text-[10px]">🌍</span>}
                                  {wh.name}
                                </p>
                              ...
                              <td>
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-[10px]" style={{ color:"var(--text-muted)" }}>{prod.sku||""}</p>
                                  </div>
                                </div>
                              </td>
                        ...
                        <div key={ws.id} className="p-3 rounded-xl" style={{ background:"var(--bg-hover)" }}>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <div className="flex gap-2 flex-wrap">
                                  ...
                                  {cf.brand  && <span className="text-[10px]" style={{ color:"var(--text-muted)" }}>· {cf.brand}</span>}
                                  {cf.hasVariants && <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{background:"rgba(99,102,241,0.1)",color:"#6366f1"}}>{variants.length} variants</span>}
                                </div>
                              </div>
                            </div>
                          ...
                          {cf.hasVariants && variants.length > 0 && (
                            <div className="mt-2 ml-10 flex flex-wrap gap-1.5">
                              {variants.map((v,i) => (
                                <span key={i} className="text-[10px] px-2 py-0.5 rounded-lg" style={{background:"var(--border)",color:"var(--text-muted)"}}>
                                  {v.name}: {v.stock||0}
                                </span>
                              ))}
                            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={transfer.fromWarehouse} onChange={e=>setTransfer(p=>({...p,fromWarehouse:e.target.value}))} className="deji-input">
              ...
              <div>
                ...
                <select value={transfer.productId} onChange={e=>setTransfer(p=>({...p,productId:e.target.value}))} className="deji-input">
                  <option value="">Select product...</option>
                  {(() => {
                    // Show products available in the selected FROM warehouse
                    const fromWH = warehouses.find(w=>w.id===transfer.fromWarehouse);
                    const availableProducts = transfer.fromWarehouse
                      ? (fromWH?.warehouseStocks||[]).filter(ws=>ws.quantity>0)
                      : inventory.filter(p=>!p.isBundle&&p.type!=="bundle");
                    const prods = transfer.fromWarehouse
                      ? availableProducts.map(ws=>ws.product).filter(Boolean)
                      : inventory.filter(p=>!p.isBundle&&p.type!=="bundle");
                    return prods.map(p=>(
                      <option key={p.id} value={p.id}>{p.name} · {p.category||""} · stock: {p.stock}</option>
                    ));
                  })()}
                </select>
                {transfer.fromWarehouse && (
                  <p className="text-[10px] mt-1" style={{color:"var(--text-muted)"}}>
                    Showing products available in selected source warehouse
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {transfer.productId && (() => {
                  ...
                  return prod && (
                    <p className="text-xs mt-1" style={{ color: Number(transfer.quantity)>prod.stock?"#ef4444":"var(--text-muted)" }}>
                      Total stock: {prod.stock} units{Number(transfer.quantity)>prod.stock&&" — ⚠ Exceeds available stock"}
                    </p>
                  );
                })()}
              </div>
              ...
              <div className="p-4 rounded-2xl space-y-3" style={{ background:"rgba(249,115,22,0.06)", border:"1px solid rgba(249,115,22,0.15)" }}>
                ...
                {Number(transfer.deliveryFee)>0&&<p className="text-xs text-orange-400">₦{Number(transfer.deliveryFee).toLocaleString()} logged automatically as a Logistics expense.</p>}
              </div>
          ...
          <div className="deji-card p-6 w-full max-w-xl max-h-[92vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
            ...
            <form onSubmit={saveWarehouse} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <input value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}
                    className="deji-input" required placeholder="e.g. Lagos Main, Dubai Hub, London 3PL"/>
                </div>
                <div>
                  <label className="deji-label">Short Code</label>
                  <input value={form.code} onChange={e=>setForm(p=>({...p,code:e.target.value}))}
                    className="deji-input" placeholder="LGS, DXB, LON"/>
                </div>
                <div>
                ...
                <div>
                  ...
                  <input value={form.city} onChange={e=>setForm(p=>({...p,city:e.target.value}))}
                    className="deji-input" placeholder="Lagos, Dubai..."/>
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="number" min="0" value={form.defaultDeliveryFee}
                    onChange={e=>setForm(p=>({...p,defaultDeliveryFee:e.target.value}))} className="deji-input" placeholder="0"/>
                </div>
                <div className="col-span-2">
                  <label className="deji-label">Address</label>
                  <input value={form.address} onChange={e=>setForm(p=>({...p,address:e.target.value}))}
                    className="deji-input" placeholder="Full warehouse address"/>
                </div>
              </div>
              ...
              <div style={{ borderTop:"1px solid var(--border)", paddingTop:"1rem" }}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="deji-label">Name</label>
                    <input value={form.contactName} onChange={e=>setForm(p=>({...p,contactName:e.target.value}))} className="deji-input" placeholder="Warehouse manager"/>
                  </div>
                  <div>
                    <label className="deji-label">Phone</label>
                    <input value={form.contactPhone} onChange={e=>setForm(p=>({...p,contactPhone:e.target.value}))} className="deji-input" placeholder="+234..."/>
                  </div>
                  <div className="col-span-2">
                    <label className="deji-label">Email</label>
                    <input type="email" value={form.contactEmail} onChange={e=>setForm(p=>({...p,contactEmail:e.target.value}))} className="deji-input" placeholder="warehouse@email.com"/>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))}
                  className="deji-input resize-none" rows={2} placeholder="e.g. Open Mon-Fri, handles fragile goods..."/>
              </div>
...
function WarehouseCard({ wh, onEdit, onDelete, onView, onSetDefault, onTransfer, isDefault = false }) {
  ...
  // Group by category for the card preview
  ...
  return (
    <div className="deji-card p-5 space-y-4" style={{ opacity:wh.isActive===false?0.55:1 }}>
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background:"var(--bg-hover)" }}>
            {info.icon}
          </div>
          <div>
            <div className="flex items-center gap-2 flex-wrap">
            ...
            <p className="text-xs" style={{ color:"var(--text-muted)" }}>
              {info.label} · {wh.city?`${wh.city}, `:""}{wh.country}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={()=>onView(wh)} className="p-1.5 rounded-lg transition-all" style={{ color:"var(--text-muted)" }} title="View stock"><Eye size={14}/></button>
          <button onClick={()=>onEdit(wh)} className="p-1.5 rounded-lg transition-all" style={{ color:"var(--text-muted)" }} title="Edit"><Edit size={14}/></button>
          <button onClick={onTransfer} className="p-1.5 rounded-lg transition-all" style={{ color:"var(--text-muted)" }} title="Transfer from here"><ArrowRightLeft size={14}/></button>
          {!isDefault && (
            <button onClick={()=>onSetDefault(wh)} className="p-1.5 rounded-lg transition-all" style={{ color:"var(--text-muted)" }} title="Set as default">⚡</button>
          )}
          <button onClick={()=>onDelete(wh)} className="p-1.5 rounded-lg transition-all" style={{ color: isDefault?"#ef444460":"var(--text-muted)" }} title={isDefault?"Cannot delete default warehouse":"Delete permanently"}>
            <Trash2 size={14}/>
          </button>
        </div>
      </div>
      ...
      {categories.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {categories.slice(0,4).map(cat=>(
            <span key={cat} className="text-[10px] px-2 py-0.5 rounded-full" style={{background:"var(--bg-hover)",color:"var(--text-muted)"}}>
              {cat}
            </span>
          ))}
      ...
      <div className="flex items-center justify-between pt-1" style={{ borderTop:"1px solid var(--border)" }}>
        ...
        {wh.currency && wh.currency!=="NGN" && (
          <span className="text-[10px] px-2 py-0.5 rounded-full font-bold" style={{ background:"rgba(59,130,246,0.1)", color:"#3b82f6" }}>{wh.currency}</span>
        )}
