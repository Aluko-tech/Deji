...
const getBaseURL = () => {
  ...
  if (window.location.hostname.includes("app.github.dev")) return "/api";
...
// Handle offline queuing + error normalization + plan limit gates
api.interceptors.response.use((res) => res, async (error) => {
  ...
  // Plan limit gate — 403 with upgrade:true fires a global event
...
// Aliases used by settings/staff/page.js
...
// ── Inventory ─────────────────────────────────────────────────────────────────
// The backend product management is under /products (product.routes.js)
// /inventory only has GET / and PUT /:id (stock only)
...
export const getLowStock            = ()       => api.get("/products?lowStock=true");
